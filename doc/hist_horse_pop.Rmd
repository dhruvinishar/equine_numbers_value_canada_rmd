...
```{r plot horses, fig.width=10, fig.height=3, fig.cap="We see the historical number of horses per province in Canada and how they have been decreasing over time"}
...
# added figure captions in the chunk options
horse_plot
```
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
# added kable for the table
knitr::kable(max_horses, caption = "Number of Horses for each province between 1940 - 1972")
```

Below we zoom in and look at the province of Alberta:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Historical number of horses from 1940 - 1972 in Alberta"}
...
As we observe above, there is a very strong negative trends in the number of horses from 1920 - 1972 from the ``r params$data`` data. (Used the data parameter here)



# References

@abutarbush2005causes

@levasseur2021surveillance
